...
public class UsersDaoJdbcImpl implements UsersDao {
    ...
    private static final String SQL_FIND_BY_TOKEN =
            "SELECT * FROM user_app WHERE  token = :token";

    // language=SQL
    private static final String SQL_FIND_BY_LOGIN =
            "SELECT * FROM user_app WHERE login = :login";

    // language=SQL
    private static final String SQL_INSERT_USER =
            "INSERT INTO user_app(login, password_hash, age, user_name) " +
                    "VALUES (?, ?, ?, ?)";

    // language=SQL
    private static final String SQL_UPDATE_TOKEN =
            "UPDATE user_app SET token = :token WHERE id = :id";
    ...
    private static final String SQL_TOKEN_IS_EXIST =
            "SELECT  EXISTS (SELECT 1 from user_app WHERE  token = :token)";
